...
public:
    ...
    DocumentScorer(const string doclens_path, const string decomposed_index_path, const size_t max_query_terms) {
        ...
        vector<float> pqcentroids{pqCentroidsArray.data<float>(),
                                  pqCentroidsArray.data<float>() + pqCentroidsArray.shape[0]};
    ...
    vector<float> compute_query_centroids_distances(const float *queries_data, const globalIdxType q_start) {
        ...
        cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasTrans, M, N, K, alpha, queries_data + q_start, K, centroids, K,
                    beta, current_scores.data(), N);
...
//        printf("after filter if\n");
        ...
        for (size_t i = 0; i < M; i++) {
            ...
            if (sorted_indexes - start_sorted >= nprobe) {
                ...
                while (current_n_probe > 0) {
                    size_t temp_argmax = std::distance(candidate_centroids_scores.begin(),
                                                       max_element(candidate_centroids_scores.begin(),
    ...
    vector<numDocsType>
    compute_hit_frequency(vector<numDocsType> &candidate_documents, const float th, const size_t k_centroids) {
    ...
    inline float compute_score_by_column_reduction(const std::vector<float> &centroid_distances, const size_t doclen,
                                                   const size_t M) {
//        printf("before compute_score_by_column_reduction\n");
        std::vector<float> maxs0(16, std::numeric_limits<float>::lowest());
        std::vector<float> maxs1(16, std::numeric_limits<float>::lowest());
...
//        printf("after compute_score_by_column_reduction\n");
    ...
    priority_queue<tuple<numDocsType, valType>, vector<tuple<numDocsType, valType>>, Compare>
    compute_topk_documents(const float *queries_data, const globalIdxType q_start, const vector<numDocsType> &doc_ids,
                           const size_t k) {
        priority_queue<tuple<numDocsType, valType>, vector<tuple<numDocsType, valType>>, Compare> min_heap;
    ...
    vector<tuple<size_t, float>>
    compute_topk_documents_2(const float *queries_data, const globalIdxType q_start, const vector<numDocsType> &doc_ids,
                             const size_t k) {
        auto heap = HeapFloats(k);
        pq.precompute_distance_table(queries_data + q_start, M);
    ...
    inline int *filter_centroids_in_scoring(const float th, const float *current_centroid_scores, const size_t doclen) {
        ...
        size_t avx_cycle_length = (doclen / 16) * 16;

        for (size_t j = 0; j < avx_cycle_length; j += 16) {
            for (size_t k = 0; k < 16; ++k) {
                if (current_centroid_scores[j + k] > th) {
                    *current_buffer = GLOBAL_INDEXES[k];
                    ++current_buffer;
                }
            }
        }
        ...
        for (size_t j = avx_cycle_length; j < doclen; ++j) {
